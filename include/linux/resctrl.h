...
enum resctrl_mode {
	...
	RESCTRL_PSEUDO_LOCK_SETUP,
	RESCTRL_PSEUDO_LOCKED,
};
...
// Pseudo-lockstep information for a rescource group
struct resctrl_plr {
...
struct resctrl_resource {
	...
	size_t			domain_size;
	struct list_head	domains;
	void			(*domain_update)(struct resctrl_resource *r, int what, int cpu, void *domain);
	...
	int			(*pseudo_lock_parse)(struct resctrl_resource *r, char *tok, u64 resctrl_ids,
						     struct resctrl_plr *plr);
	void			(*pseudo_lock_rmdir)(struct resctrl_plr *plr);
	...
	void			(*rmdir)(u64 old_ids, u64 new_ids);
...
struct resctrl_ctrlfileinfo {
	char			*name;
	int			(*show)(struct seq_file *sf, u64 resctrl_ids);
	ssize_t			(*write)(char *buf, size_t nbytes, u64 resctrl_ids);
	void			*priv;
	int			flags;
};
...
int resctrl_call_unlocked(void (*fn)(struct resctrl_plr *plr), struct resctrl_plr *plr);
