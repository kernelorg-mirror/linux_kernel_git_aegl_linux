...
extern int resctrl_pseudolock_available;
int resctrl_activate(struct resctrl_resource *r);
void resctrl_deactivate(struct resctrl_resource *r, bool do_ctrl, struct list_head *h);
...
void resctrl_move_group_tasks(struct resctrl_group *from, struct resctrl_group *to,
			      struct cpumask *mask);
bool resctrl_tasks_assigned(struct resctrl_group *rg);
