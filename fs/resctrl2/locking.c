...
/*
 * Pseudolock module needs to release resctrl_mutex and reacquire
 * it around code that creates debugfs entries and character device.
 *
 * Non-zero return if the group was deleted during the unlocked call.
 */
int resctrl_call_unlocked(void (*fn)(struct resctrl_plr *plr), struct resctrl_plr *plr)
{
	...
	return rni->flags & RESCTRL_DELETED;
}
EXPORT_SYMBOL_GPL(resctrl_call_unlocked);
