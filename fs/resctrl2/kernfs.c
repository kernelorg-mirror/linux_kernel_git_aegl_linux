					   ...
					   umode_t mode, int type)
{
	...
	case RESCTRL_CTRLFILE: size += sizeof(struct ctrl_file_info); break;
	default:
		return NULL;
	}
				  ...
				  size_t nbytes, loff_t off)
{
	...
	case RESCTRL_COREFILE:
		cfi = (struct core_file_info *)&rni->priv;
		ret = cfi->write(buf, nbytes, cfi->rg, of, &clean_list);
		break;
	case RESCTRL_CTRLFILE:
		tfi = (struct ctrl_file_info *)&rni->priv;
