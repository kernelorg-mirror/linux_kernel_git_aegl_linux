...
#define MBM_POLL_THRESHOLD	10000000	// MBytes/sec
...
static int mbm_poll(void *v)
{
	...
	while (ri.mydomain->rmid_polling && !kthread_should_stop()) {
		...
		if (ri.total_chunks > mbm_poll_threshold_chunks)
...
static void domain_update(struct resctrl_resource *r, int what, int cpu, void *domain)
{
	...
	if (what == RESCTRL_DOMAIN_DELETE) {
		...
		if (kn) {
			sprintf(name, monitor.mon_domain_dir, m->id);
...
static ssize_t mbm_poll_threshold_write(char *buf, size_t nbytes)
{
	unsigned int mbytes;
	int ret;

	ret = kstrtouint(buf, 0, &mbytes);
	if (ret)
		return ret;
	if (mbytes > MBM_POLL_THRESHOLD)
		return -EINVAL;

	mbm_poll_threshold = mbytes;
	mbm_poll_threshold_chunks = ((u64)mbytes << 20);
	do_div(mbm_poll_threshold_chunks, upscale);

	return nbytes;
}

RESCTRL_FILE_DEF(max_threshold_occupancy, "%d\n")
RESCTRL_FILE_DEF(mbm_poll_threshold, "%d\n")
...
static struct resctrl_fileinfo monitor_files[] = {
	...
	{
		.name 	= "mbm_poll_threshold",
		.show 	= mbm_poll_threshold_show,
		.write 	= mbm_poll_threshold_write,
	},
	{
		.name	= "mon_features",
...
static int __init rdt_monitor_init(void)
{
	...
	max_threshold_occupancy = llc_busy_threshold * upscale;
	mbm_poll_threshold_chunks = (mbm_poll_threshold << 20) / upscale;
