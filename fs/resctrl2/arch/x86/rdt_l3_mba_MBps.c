...
static bool validate_bandwidth(char *buf, struct bandwidth_values *c)
{
	unsigned long val;
	int ret;
...
next:
	...
	list_for_each_entry(m, &r->domains, list) {
		if (m->id != dom_id)
			continue;
		tvalues = m->bandwidth_values;
		if (!validate_bandwidth(dom, tvalues + closid))
			return -EINVAL;
		goto next;
	}
	return -EINVAL;
...
static void check_one(u64 resctrl_ids, void *v)
{
	...
	got = rdt_rmid_read(m->id, rmid, EV_LOCRATE) >> 20;
...
static void domain_update(struct resctrl_resource *r, int what, int cpu, void *domain)
{
	...
	if (what == RESCTRL_DOMAIN_ADD || what == RESCTRL_DOMAIN_DELETE) {
		...
		if (what == RESCTRL_DOMAIN_ADD) {
			m->cpu = cpumask_first(&m->cpu_mask);
			m->kthread = kthread_create_on_cpu(checkbw, m, m->cpu, "mba_MBps %d");
