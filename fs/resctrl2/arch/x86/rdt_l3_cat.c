...
static void show(struct resctrl_resource *r, struct seq_file *sf, u64 resctrl_ids)
{
	...
	list_for_each_entry(m, &r->domains, list) {
		cbm = m->cbm_masks;
		seq_printf(sf, "%s%d=%llx", sep, m->id, cbm[closid].now);
		sep = ";";
	}
	seq_puts(sf, "\n");
}

static long get_cache_slice_size(int scope, struct mydomain *m, u64 cbm)
{
	...
	return m->cache_size / bits * num_b;
}

static void size(struct resctrl_resource *r, struct seq_file *sf, u64 resctrl_ids)
{
	int closid = (resctrl_ids >> 32);
	struct cbm_masks *cbm;
	struct mydomain *m;
	...
	list_for_each_entry(m, &r->domains, list) {
		cbm = m->cbm_masks;
		cache_size = get_cache_slice_size(r->scope, m, cbm[closid].now);
		seq_printf(sf, "%s%d=%ld", sep, m->id, cache_size);
		sep = ";";
	}
	seq_puts(sf, "\n");
}
...
static bool validate_mask(struct mydomain *m, char *buf, struct cbm_masks *c, int closid, bool plock)
{
...
#ifdef PSEUDO_LOCK
	if (plock) {
		if (m->pseudolock_mask) {
			resctrl_last_cmd_puts("Only one pseudo-lock group per domain\n");
			return false;
		}

		// Pseudlock groups can only claim unused bits
		busy_bits = shareable_bits | m->exclusive_mask;
		...
		if (val & busy_bits)
			return false;
		m->pseudolock_mask = val;
		goto ok;
	} else if (val & m->pseudolock_mask) {
		return false;
	}
#endif
...
static int __parse(struct resctrl_resource *r, char *line, u64 resctrl_ids, struct resctrl_plr *plr, bool plock)
{
	int closid = (resctrl_ids >> 32);
	struct cbm_masks *cbm;
	char *dom = NULL, *id;
...
next:
	...
	list_for_each_entry(m, &r->domains, list) {
		if (m->id != dom_id)
			continue;
		cbm = m->cbm_masks;
		if (!validate_mask(m, dom, cbm, closid, plock))
			return -EINVAL;
#ifdef PSEUDO_LOCK
		if (plock) {
			long size;
			...
			plr->closid = closid;
			plr->cbm = cbm[closid].staged;
			size = get_cache_slice_size(r->scope, m, plr->cbm);

			if (size > KMALLOC_MAX_SIZE) {
				resctrl_last_cmd_puts("Requested region exceeds maximum size\n");
				//return -E2BIG;
				size = 64*1024; // TODO fake smaller allocation
			}
			plr->d = m;
			plr->size = size;
			return 0;
...
{
	return __parse(r, line, resctrl_ids, NULL, false);
}

#ifdef PSEUDO_LOCK
static int pseudo_lock_parse(struct resctrl_resource *r, char *line, u64 resctrl_ids,
			     struct resctrl_plr *plr)
{
	int ret;

	ret = __parse(r, line, resctrl_ids, plr, true);
	if (!ret) {
		plr->r = r;
		ret = pseudo_lock_create(plr);
	}

	return ret;
}

static void pseudo_lock_rmdir(struct resctrl_plr *plr)
{
	struct mydomain *m = plr->d;

	pseudo_lock_remove(plr);
	m->pseudolock_mask = 0;
}
#endif
...
static bool setmode(struct resctrl_resource *r, u64 resctrl_ids, enum resctrl_mode mode)
{
	...
	if (state[closid] == RESCTRL_FREE) {
		if (mode != RESCTRL_SHARED)
			return false;
		list_for_each_entry(m, &r->domains, list) {
			if (m->exclusive_mask || m->pseudolock_mask)
...
static struct resctrl_resource cat = {
	...
	.parse		= parse,
#ifdef PSEUDO_LOCK
	.pseudo_lock_parse = pseudo_lock_parse,
	.pseudo_lock_rmdir = pseudo_lock_rmdir,
#endif
