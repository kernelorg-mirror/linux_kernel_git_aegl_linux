...
static long get_cache_slice_size(struct resctrl_domain *d, u64 cbm)
{
...
static void size(struct resctrl_resource *r, struct seq_file *m, u64 resctrl_ids)
{
	...
	list_for_each_entry(d, &r->domains, list) {
		cbm = get_mydomain(d)->cbm_masks;
		cache_size = get_cache_slice_size(d, cbm[closid].now);
...
static bool validate_mask(struct resctrl_domain *d, char *buf, struct cbm_masks *c, int closid, bool plock)
{
...
next:
	...
	list_for_each_entry(d, &r->domains, list) {
		...
		m = get_mydomain(d);
		cbm = m->cbm_masks;
		if (!validate_mask(d, dom, cbm, closid, plock))
			return -EINVAL;
		if (plock) {
			long size;
			...
			size = get_cache_slice_size(d, plr->cbm);
...
{
	struct mydomain *m = get_mydomain(plr->d);
...
static void update_msrs(void *info)
{
	...
	for (int i = 0; i < cat.num_alloc_ids; i++) {
		if (mi->cbm[i].need_update) {
			...
			wrmsrl(mi->msr_base + i, mi->cbm[i].now);
		}
	}
}
...
static bool setmode(struct resctrl_resource *r, u64 resctrl_ids, enum resctrl_mode mode)
{
	...
	} else if (mode == RESCTRL_PSEUDO_LOCK_SETUP) {
		return true;
...
static void show_bits(struct seq_file *sf, struct mydomain *m)
{
	...
	for (i = m->cbm_len; i >= 0; i--) {
		...
		if (m->exclusive_mask & BIT(i))
			idx |= BIT(0);
		if (m->pseudolock_mask & BIT(i))
			idx |= BIT(3);
		seq_putc(sf, "0ES!H!X!P!!!!!!!"[idx]);
	}
}
