...
int resctrl_mkdir(struct kernfs_node *parent_kn, const char *name, umode_t mode)
{
	...
	case RESCTRL_MONGROUP:
		...
		if (prg->mode == RESCTRL_PSEUDO_LOCK_SETUP || prg->mode == RESCTRL_PSEUDO_LOCKED) {
			kfree(rni);
			resctrl_last_cmd_puts("Pseudo-locking in progress\n");
			ret = -EINVAL;
			goto unlock;
		}
		rg->parent = prg;
...
void resctrl_rmdir_all_sub(struct list_head *h)
{
	...
	list_for_each_entry_safe(rg, tmp, &all_ctrl_groups, list) {
		...
		if (rg->mode == RESCTRL_PSEUDO_LOCKED) {
			rg->plr.r->pseudo_lock_rmdir(&rg->plr);
			goto remove;
		}
		   ...
		   const char *new_name)
{
	...
	if (new_prg->mode == RESCTRL_PSEUDO_LOCK_SETUP || new_prg->mode == RESCTRL_PSEUDO_LOCKED) {
		resctrl_last_cmd_puts("Pseudo-locking in progress\n");
		ret = -EINVAL;
		goto out;
	}
